 ...
 */
@Service
public class BJHISubmitDPPJob extends SuperJob implements SubmitJob {
	
	public BJHISubmitDPPJob(){
		//设置为报送群组
	...
	public boolean submit(String startDate, String endDate) throws CISException,Exception {
		//获取平台
		System.out.println("=================北京健康险报送批处理-保全信息同步上传开始 =================");
		try{
			
			String bussType = "GetEndorsementInfo";
			String bussNo = "";
			return submit(new BigDecimal(UMMPContext.SUBMIT_PLAT_BJHI), bussType, startDate, endDate, bussNo);
		}catch(Exception e){
			throw e;
		}finally{
			System.out.println("=================北京健康险报送批处理-保全信息同步上传结束 =================");	
		}
	}
}
