 ...
 */
@Service
public class BJAISubmitGBSDPPJob extends SuperJob implements SubmitJob {
	
	public BJAISubmitGBSDPPJob(){
		//设置为报送群组
	...
	public boolean submit(String startDate, String endDate) throws CISException,Exception {
		//获取平台
		System.out.println("=================北京意外险报送批处理-团险保全信息同步上传开始=================");
		try{
			
			String bussType = "GroupEndorsementInfo";
			String bussNo = "";
			return submit(new BigDecimal(UMMPContext.SUBMIT_PLAT_BJAI), bussType, startDate, endDate, bussNo);
		}catch(Exception e){
			throw e;
		}finally{
			System.out.println("=================北京意外险报送批处理-团险保全信息同步上传结束 =================");	
		}
	}
}
