 ...
 * @company SINOSOFT
 */
public class BJHIExtractDataJob extends SuperJob {

	ExtractService service = null;

	public BJHIExtractDataJob() {
		this.setJobGroup("JOB_EXTRACT");  // 设置为提数群组
	}

	/**
	 * 提数，提完数日期置后：提数日期在T_DEF_CODE_DICT中配置 
	 *     code_type=SYS_VAR
	...
	public boolean excute() throws Exception {
		System.out.println("========北京健康险数据提取批处理开始========");
		try {
			BigDecimal comId = new BigDecimal(UMMPContext.SUBMIT_PLAT_BJHI);
			boolean result = extract(comId);
			System.out.println("========北京健康险数据提取批处理结果：    " + result);
		} catch (Exception e) {
			DBContextHolder.recover();
			throw e;
		} finally {
			System.out.println("========北京健康险数据提取批处理结束========");
		}
		return true;
	}
