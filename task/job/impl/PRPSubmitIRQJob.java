...
/**
 * 保单登记报送批处理:IncResultQuery
 * @author Darker.Wang
 * @date 2016-8-18 下午2:06:05
 * @company SINOSOFT
 */
@Service
public class PRPSubmitIRQJob extends SuperJob implements SubmitJob {
	
	public PRPSubmitIRQJob(){
		//设置为报送群组
	...
	public boolean submit(String startDate, String endDate) throws Exception{
		//获取平台
		System.out.println("================= 保单登记数据报送,增量上载查询批处理开始 =================");
		try{
			String bussType = "IncResultQuery";
			String bussNo = "";
			return submit(new BigDecimal(UMMPContext.SUBMIT_PLAT_PRP), bussType, startDate, endDate, bussNo);
		}catch(Exception e){
			throw e;
		}finally{
			System.out.println("================= 保单登记数据报送，增量上载查询批处理结束 =================");	
		}
	}
}
