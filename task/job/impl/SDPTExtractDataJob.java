 ...
 * @company SINOSOFT
 */
public class SDPTExtractDataJob extends SuperJob{

	ExtractService service = null;
	public SDPTExtractDataJob(){
		//设置为提数群组
	...
	public boolean excute() throws Exception {
		System.out.println("================= 山东预警平台数据提取批处理开始 =================");
		try{
			BigDecimal comId = new BigDecimal(UMMPContext.SUBMIT_PLAT_SDPT);
			boolean result = extract(comId);
			System.out.println("- 山东预警平台数据提取批处理结果："+result);
		}catch(Exception e){
			DBContextHolder.recover();
			throw e;
		}finally{
			System.out.println("================= 山东预警平台数据提取批处理结束 =================");
		}
		return true;
	}
