 ...
 */
@Service
public class PRPSubmitFUJob extends SuperJob implements SubmitJob {
	
	public PRPSubmitFUJob(){
		//设置为报送群组
	...
	public boolean submit(String startDate, String endDate) throws CISException,Exception {
		//获取平台
		System.out.println("================= 保单登记数据报送,重报数据上传成功通知开始 =================");
		try{
			String bussType = "FaiUpload";
			String bussNo = "";
			return submit(new BigDecimal(UMMPContext.SUBMIT_PLAT_PRP), bussType, startDate, endDate, bussNo);
		}catch(Exception e){
			throw e;
		}finally{
			System.out.println("================= 保单登记数据报送，重报数据上传成功通知结束 =================");	
		}
	}
}
