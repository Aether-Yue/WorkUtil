 ...
 */
@Service
public class BJHISubmitDPLJob extends SuperJob implements SubmitJob {
	
	public BJHISubmitDPLJob(){
		//设置为报送群组
	...
	public boolean submit(String startDate, String endDate) throws CISException,Exception {
		//获取平台
		System.out.println("=================北京健康险报送批处理-理赔信息同步上传开始 =================");
		try{
			
			String bussType = "GetClaimInfo";
			String bussNo = "";
			return submit(new BigDecimal(UMMPContext.SUBMIT_PLAT_BJHI), bussType, startDate, endDate, bussNo);
		}catch(Exception e){
			throw e;
		}finally{
			System.out.println("=================北京健康险报送批处理-理赔信息同步上传结束 =================");	
		}
	}
}
