 ...
 */
@Service
public class JSAPSubmitC01Job extends SuperJob implements SubmitJob {
	
	public JSAPSubmitC01Job(){
		//设置为报送群组
	...
	public boolean submit(String startDate, String endDate) throws Exception {
		...
		//获取平台
		System.out.println("================= 江苏中介报送批处理开始 =================");
		/**
		 * 中介机构校验_承保:C01CB 
		 * 中介机构校验_保全:C01BQ
		**/
		...
		try{
			for(int i = 0; i <busstypes.length; i++){
				...
				String bussNo = "";
				boolean result = submit(new BigDecimal(UMMPContext.SUBMIT_PLAT_JSAP), busstype, startDate, endDate, bussNo);
			...
			System.out.println("================= 江苏中介数据报送批处理结束 =================");	
		}
		return true;
	}
}
